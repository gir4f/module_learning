  ...
  <header class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <h1 class="mt-3 max-w-4xl text-3xl font-black leading-tight text-brand-navy dark:text-cyan-200 sm:text-4xl">{{ module.title }}</h1>
        <p v-if="module.description" class="mt-4 max-w-prose text-base leading-8 text-slate-600 dark:text-slate-300">{{ module.description }}</p>
      </div>
      <div class="flex flex-wrap gap-2 no-print xl:hidden">
        <button
          type="button"
          class="inline-flex items-center gap-2 rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-brand-teal hover:text-brand-navy focus:outline-none focus-visible:ring-4 focus-visible:ring-cyan-100 dark:border-slate-700 dark:text-slate-200 dark:focus-visible:ring-cyan-950"
          @click="shareModule"
        >
        ...
        <button
          type="button"
          class="inline-flex items-center gap-2 rounded-md bg-brand-teal px-3 py-2 text-sm font-semibold text-white transition hover:bg-brand-teal-dark focus:outline-none focus-visible:ring-4 focus-visible:ring-cyan-100 dark:focus-visible:ring-cyan-950"
          @click="printModule"
        >
