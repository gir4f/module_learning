    ...
    <div class="flex flex-1 flex-col p-4 sm:p-5 xl:p-4">
      ...
      <div class="mt-4 flex gap-2 overflow-x-auto pb-1 sm:flex-wrap sm:overflow-visible sm:pb-0">
        <span class="shrink-0 rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300">
          {{ module.details.length }} varian produk
        </span>
        <span class="shrink-0 rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300">
...
const { module, index, search } = defineProps<{
...
const icon = computed(() => moduleIcon(module))
...
const description = computed(() => module.description || module.keywords || 'Dokumentasi produk dan komponen tersedia di dalam modul.')
const highlightedTitle = computed(() => highlightParts(module.title, search || ''))
const highlightedDescription = computed(() => highlightParts(description.value, search || ''))
