<template>
  <header class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    ...
    <dl class="mt-5 grid gap-3 border-t border-slate-100 pt-4 dark:border-slate-800 sm:grid-cols-4">
      ...
      <div>
        <dt class="text-xs font-semibold uppercase text-slate-500">Sections</dt>
        <dd class="mt-1 text-sm font-bold text-slate-900 dark:text-slate-100">{{ module.details.length }}</dd>
      </div>
      <div>
        <dt class="text-xs font-semibold uppercase text-slate-500">Components</dt>
    ...
    <p class="mt-3 text-xs text-slate-500 dark:text-slate-400">Diperbarui {{ updatedAt }}</p>
  </header>
</template>
